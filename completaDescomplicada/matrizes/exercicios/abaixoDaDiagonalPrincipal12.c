/**
 * 12) Faça um programa que leia uma matriz de tamanho 
 * 6 × 6. Calcule e imprima a soma dos elementos dessa 
 * matriz que estão abaixo da diagonal principal.
...
// procedimento para achar e calcular a soma
// dos valores abaixo da diagonal principal
void abaixoDaDiagonal(int matriz[][SIZE])
{
   // variáveis
   int i, j; 
   int soma = 0;
   ...
   for ( i = 0; i < SIZE; i++)
   {
      for(j = 0; j < SIZE; j++)
         // verificar se linha(i) 
         // é maior que a coluna(j)
         if(i > j) { // se verdade
            // imprima os valores abaixo da
            // diagonal principal
            printf("%4d", matriz[i][j]);
            // e some esses valores
            soma += matriz[i][j];
...
int main()
{
   ...
   // exibe os valores abaixo da diagonal principal
   printf("Abaixo da diagonal principal:\n");
   abaixoDaDiagonal(matriz);
